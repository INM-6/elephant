...
def _dither_spikes_with_refractory_period(
    ...
    refractory_period = np.min(np.diff(spiketrain.magnitude), initial=refractory_period)
...
def _dither_spikes(
    spiketrain: neo.SpikeTrain, dither: float, n_surrogates: int, edges: bool
) -> np.array:
    units = spiketrain.units
...
def dither_spikes(
    ...
    if not refractory_period:
        dithered_spiketrains = _dither_spikes(spiketrain, dither, n_surrogates, edges)
...
def randomise_spikes(spiketrain, n_surrogates=1, decimals=None):
    ...
    sts = (
        (spiketrain.t_stop - spiketrain.t_start)
...
def shuffle_isis(spiketrain, n_surrogates=1, decimals=None):
    ...
    for surrogate_id in range(n_surrogates):
        surr_times = (
            np.cumsum(np.random.permutation(isis)) * spiketrain.units
            + spiketrain.t_start
        )
        sts.append(
            neo.SpikeTrain(
...
@deprecated_alias(n="n_surrogates")
def dither_spike_train(spiketrain, shift, n_surrogates=1, decimals=None, edges=True):
    """
    ...
    surr = (
        ...
        + 2 * shift * np.random.random_sample((n_surrogates, 1))
        - shift
    )
    ...
    if edges is False:
        ...
        surr = (
            np.minimum(
                np.maximum(
                    surr.simplified.magnitude, spiketrain.t_start.simplified.magnitude
                ),
        ...
        surr = [
            np.sort(s[np.all([s >= tstart, s < tstop], axis=0)]) * pq.s
...
def jitter_spikes(spiketrain, bin_size, n_surrogates=1):
    ...
    bin_ids = np.array(
        (spiketrain.view(pq.Quantity) / bin_size).rescale(pq.dimensionless).magnitude,
        dtype=int,
    )
...
def bin_shuffling(
    spiketrain, max_displacement, bin_size=None, n_surrogates=1, sliding=False
):
    """
    ...
    for surrogate_id in range(n_surrogates):
        ...
        if sliding:
            for window_position in range(st_length - displacement_window):
                ...
                np.random.shuffle(
                    surrogate_spiketrain[
                        window_position : window_position + displacement_window
                    ]
                )
        else:
            ...
            if windows_remainder != 0:
                np.random.shuffle(surrogate_spiketrain[windows * displacement_window :])
...
def _continuous_time_bin_shuffling(
    ...
    bin_indices = np.split(bin_indices, split_indices)

    surrogate_spiketrains = []
    for surrogate_id in range(n_surrogates):
        surrogate_bin_indices = np.empty(shape=len(bin_indices), dtype=np.ndarray)
        for i, bin_indices_slice in enumerate(bin_indices):
            ...
            surrogate_bin_indices[i] = (
                random_indices[bin_indices_slice - window_start] + window_start
            )
        ...
        surrogate_spiketrain = surrogate_spiketrain[
            np.all(
                (surrogate_spiketrain > t_start, surrogate_spiketrain < t_stop), axis=0
            )
        ]
...
class JointISI(object):
    ...
    def refr_period(self):
        warnings.warn(
            "'.refr_period' is deprecated; use '.refractory_period'", DeprecationWarning
        )
    ...
    def joint_isi_histogram(self):
        ...
        if not self.isi_dithering:
            joint_isi_histogram = np.histogram2d(
                ...
                range=[[0.0, self.truncation_limit], [0.0, self.truncation_limit]],
            )[0]
        else:
            isi_histogram = np.histogram(
                isis, bins=self.n_bins, range=[0.0, self.truncation_limit]
            )[0]
        ...
        if self.sigma:
            if self.cutoff:
                ...
                joint_isi_histogram[start_index:, start_index:] = gaussian_filter(
    ...
    def dithering(self, n_surrogates=1):
        ...
        for _ in range(n_surrogates):
            ...
            dithered_st = (
                self.spiketrain[0].magnitude + np.r_[0.0, np.cumsum(dithered_isi)]
            )
        ...
        if self.method == "fast":
            ...
            for double_index in range(self.n_bins):
                ...
                jisih_cum = self._normalize_cumulative_distribution(
                    np.r_[0.0, np.cumsum(diagonal)]
                )
                self._jisih_cumulatives.append(jisih_cum)
            self._jisih_cumulatives = np.array(self._jisih_cumulatives, dtype=object)
        else:
    ...
    def _window_cumulatives(self, rotated_jisih):
        ...
        jisih_cumulatives = np.zeros(
            (self.n_bins, self.n_bins, 2 * self._max_change_index + 1)
        )
        for curr_isi_id in range(self.n_bins):
            for next_isi_id in range(self.n_bins - curr_isi_id):
                ...
                normalized_cum = self._normalize_cumulative_distribution(cum_slice)
    ...
    def _window_diagonal_cumulatives(self, rotated_jisih):
        ...
        jisih_diag_cums = np.zeros(
            (self.n_bins, self.n_bins + 2 * self._max_change_index)
        )

        # double_index corresponds to the sum of the indices for the previous
        # and the subsequent ISI.
        for double_index in range(self.n_bins):
            anti_diagonal = np.diagonal(rotated_jisih, -self.n_bins + double_index + 1)

            right_padding = (
                jisih_diag_cums.shape[1] - len(anti_diagonal) - self._max_change_index
            )
            ...
            padded_cumulated_diagonal = np.pad(
                ...
                mode="constant",
                constant_values=(0.0, cumulated_diagonal[-1]),
            )
    ...
    def _get_dithered_isi(self, isi_to_dither):
        ...
        for start in range(sampling_rhythm):
            ...
            for i in range(start, number_of_isis - 1, sampling_rhythm):
                step = self._get_dithering_step(dithered_isi, dithered_isi_indices, i)
                dithered_isi[i] += step
                dithered_isi[i + 1] -= step

        return dithered_isi

    def _get_dithering_step(self, dithered_isi, dithered_isi_indices, i):
        ...
        if double_index < self.n_bins:
            if self.method == "fast":
                cum_dist_func = self._jisih_cumulatives[double_index]
                compare_isi = self._index_to_isi(curr_isi_id + 1)
            else:
                cum_dist_func = self._jisih_cumulatives[curr_isi_id][next_isi_id]
                compare_isi = self._max_change_isi

            if cum_dist_func[-1] > 0.0:
        ...
        return self._uniform_dither_not_jisi_movable_spikes(
            dithered_isi[i], dithered_isi[i + 1]
        )

    def _uniform_dither_not_jisi_movable_spikes(self, curr_isi, next_isi):
...
def trial_shifting(spiketrains, dither, n_surrogates=1):
    ...
    t_starts = [
        single_trial_st.t_start.simplified.magnitude for single_trial_st in spiketrains
    ]
    t_stops = [
        single_trial_st.t_stop.simplified.magnitude for single_trial_st in spiketrains
    ]
    sampling_rates = [single_trial_st.sampling_rate for single_trial_st in spiketrains]
    spiketrains = [
        single_trial_st.simplified.magnitude for single_trial_st in spiketrains
    ]
...
def _trial_shifting_of_concatenated_spiketrain(
    ...
    t_starts = t_start + np.arange(n_trials) * (trial_length + trial_separation)
    t_stops = t_starts + trial_length
    spiketrains = spiketrain.simplified.magnitude
    spiketrains = [
        spiketrains[
            (spiketrains >= t_starts[trial_id]) & (spiketrains <= t_stops[trial_id])
        ]
...
def surrogates(
    spiketrain, n_surrogates=1, method="dither_spike_train", dt=None, **kwargs
):
    """
    ...
    if isinstance(spiketrain, list):
        if not isinstance(spiketrain[0], neo.SpikeTrain):
            raise TypeError(
                "spiketrain must be an instance neo.SpikeTrain or"
    ...
    if method == "dither_spikes_with_refractory_period":
        warnings.warn(
            "'dither_spikes_with_refractory_period' is deprecated "
            "in favor of 'dither_spikes'",
            DeprecationWarning,
        )
    ...
    surrogate_types = {
        ...
        "joint_isi_dithering": lambda n: JointISI(spiketrain, **kwargs).dithering(n),
        "isi_dithering": lambda n: JointISI(
            spiketrain, isi_dithering=True, **kwargs
        ).dithering(n),
    }

    if method not in surrogate_types.keys():
        raise ValueError(
            "Specified surrogate method ('{}') " "is not valid".format(method)
        )
    method = surrogate_types[method]

    if dt is None and method not in (randomise_spikes, shuffle_isis):
        raise ValueError(
            f"'{method.__name__}' method requires 'dt' parameter " f"to be set"
        )

    if method in (dither_spike_train, dither_spikes):
        return method(spiketrain, dt, n_surrogates=n_surrogates, **kwargs)
    ...
    if method is trial_shifting:
        if isinstance(spiketrain, list):
            return method(spiketrain, dither=dt, n_surrogates=n_surrogates)
