...
def estimate_csd(
    lfp, coordinates="coordinates", method=None, process_estimate=True, **kwargs
):
    """
    ...
    for coord in coordinates:
        try:
            ...
            raise AttributeError(
                "No units given for electrode spatial \
            coordinates"
            )
    ...
    if method in kernel_methods:
        ...
        if method in icsd_methods:
            try:
                ...
                raise ValueError(
                    f"Parameter diam must be specified for iCSD "
                    f"methods: {', '.join(icsd_methods)}"
                )

        if "f_type" in kwargs:
            if (kwargs["f_type"] != "identity") and (kwargs["f_order"] is None):
                raise ValueError(
                    f"The order of {kwargs['f_type']} filter must" f" be specified"
                )

        csd_method = getattr(icsd, method)  # fetch class from icsd.py file
        csd_estimator = csd_method(
            lfp=lfp.T.magnitude * lfp.units,
...
def generate_lfp(
    ...
    if dim == 1:
        ...
        pots /= 2.0 * sigma  # eq.: 26 from Potworowski et al
        ele_pos = x_positions
    elif dim == 2:
        ...
        pots = integrate_2D(x_positions, y_positions, x, y, csd, h, chrg_x, chrg_y)
        pots /= 2 * np.pi * sigma
    ...
    asig = neo.AnalogSignal(
        np.expand_dims(pots, axis=0), sampling_rate=pq.kHz, units="mV"
    )
