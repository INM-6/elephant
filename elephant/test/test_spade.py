...
class SpadeTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.cpp = compound_poisson_process(
            rate=3 * pq.Hz, amplitude_distribution=self.amplitude, t_stop=5 * pq.s
        )
        ...
        self.patt1_times = neo.SpikeTrain(
            np.arange(0, 1000, 1000 // self.n_occ1) * pq.ms, t_stop=self.t_stop * pq.ms
        )
        ...
        self.elements_msip = [
            ...
            list(
                range(
                    self.n_spk1 + self.n_spk2, self.n_spk1 + self.n_spk2 + self.n_spk3
                )
            ),
        ]
        self.occ1 = np.unique(
            conv.BinnedSpikeTrain(self.patt1_times, self.bin_size).spike_indices[0]
        )
        self.occ2 = np.unique(
            conv.BinnedSpikeTrain(self.patt2_times, self.bin_size).spike_indices[0]
        )
        self.occ3 = np.unique(
            conv.BinnedSpikeTrain(self.patt3_times, self.bin_size).spike_indices[0]
        )
    ...
    def test_spade_cpp(self):
        output_cpp = spade.spade(
            self.cpp,
            self.bin_size,
            1,
            approx_stab_pars=dict(
                n_subsets=self.n_subset, stability_thresh=self.stability_thresh
            ),
            n_surr=self.n_surr,
            alpha=self.alpha,
    ...
    def test_spade_spectrum_cpp(self):
        # Computing Spectrum
        spectrum_cpp = spade.concepts_mining(self.cpp, self.bin_size, 1, report="#")[0]
        # Check spectrum
        assert_array_equal(
            spectrum_cpp,
            [
    ...
    def test_parameters(self):
        ...
        assert_equal(
            occ_msip_min_occ, [occ for occ in self.occ_msip if len(occ) >= self.min_occ]
        )
    ...
    def test_fpgrowth_fca(self):
        ...
        binary_matrix = (
            conv.BinnedSpikeTrain(self.patt1, self.bin_size)
        ...
        mining_results_fpg = spade._fpgrowth(transactions, rel_matrix=rel_matrix)
        print("################################################################")
        ...
        assert_array_equal(
            sorted(mining_results_ffca[0][0]), sorted(mining_results_fpg[0][0])
        )
        assert_array_equal(
            sorted(mining_results_ffca[0][1]), sorted(mining_results_fpg[0][1])
        )
    ...
    def test_spectrum(self):
        # test 2d spectrum
        spectrum = spade.concepts_mining(
            self.patt1, self.bin_size, self.winlen, report="#"
        )[0]
        # test 3d spectrum
        assert_array_equal(spectrum, [[len(self.lags1) + 1, self.n_occ1, 1]])
        spectrum_3d = spade.concepts_mining(
            self.patt1, self.bin_size, self.winlen, report="3d#"
        )[0]
        assert_array_equal(
            spectrum_3d, [[len(self.lags1) + 1, self.n_occ1, max(self.lags1), 1]]
        )

    def test_spade_raise_error(self):
        # Test list not using neo.Spiketrain
        self.assertRaises(
            TypeError, spade.spade, [[1, 2, 3], [3, 4, 5]], 1 * pq.ms, 4, stat_corr="no"
        )
        self.assertRaises(
            TypeError, spade.concepts_mining, [[1, 2, 3], [3, 4, 5]], 1 * pq.ms, 4
        )
        ...
        self.assertRaises(
            ValueError, spade.approximate_stability, (), np.ndarray([]), n_subsets=-3
        )
    ...
    def test_pattern_set_reduction(self):
        ...
        concepts = spade.pattern_set_reduction(
            [concept3, concept4], ns_signatures=[], winlen=winlen, spectrum="#"
        )
        self.assertEqual(concepts, [concept3, concept4])

    @unittest.skipUnless(HAVE_STATSMODELS, "'fdr_bh' stat corr requires statsmodels")
    def test_signature_significance_fdr_bh_corr(self):
        ...
        sig_spectrum = spade.test_signature_significance(
            ...
            concepts=([[(2, 3), (1, 2, 3)], [(2, 4), (1, 2, 3, 4)]]),
            alpha=0.15,
            winlen=1,
            corr="fdr_bh",
        )
        self.assertEqual(sig_spectrum, [(2.0, 3.0, False), (2.0, 4.0, True)])
