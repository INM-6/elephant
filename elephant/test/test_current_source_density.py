...
class CSD1D_TestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.params = {
            "DeltaiCSD": {"sigma_top": 0.0 * pq.S / pq.m, "diam": 500e-6 * pq.m},
            "StepiCSD": {
                "sigma_top": 0.0 * pq.S / pq.m,
                "tol": 1e-12,
                "diam": 500e-6 * pq.m,
            },
            "SplineiCSD": {
            ...
            "StandardCSD": {},
            "KCSD1D": {"h": 50.0, "Rs": np.array((0.1, 0.25, 0.5))},
        }

    def test_validate_inputs(self):
        self.assertRaises(TypeError, self.csd_method, lfp=[[1], [2], [3]])
        self.assertRaises(
            ValueError, self.csd_method, lfp=self.lfp, coordinates=self.ele_pos * pq.mm
        )
        ...
        # bad method name
        self.assertRaises(
            ValueError, self.csd_method, lfp=self.lfp, method="InvalidMethodName"
        )
...
class CSD2D_TestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.params = {
            "KCSD2D": {"sigma": 1.0, "Rs": np.array((0.1, 0.25, 0.5))}
        }  # Input dictionaries for each method
...
class GenerateLfpTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.two_dimensional = np.linspace(0, 10, 2304).reshape(2304, 1)
