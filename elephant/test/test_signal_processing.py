...
class PairwiseCrossCorrelationTest(unittest.TestCase):
    ...
    def test_cross_correlation_freqs(self):
        ...
        for freq in freq_arr:
            signal[:, 0] = np.sin(2.0 * np.pi * freq * self.times)
            signal[:, 1] = np.cos(2.0 * np.pi * freq * self.times)
            signal[:, 2] = np.cos(2.0 * np.pi * freq * self.times + 0.2)
            ...
            assert_array_almost_equal(
                rho.magnitude[:, 0], np.sin(2.0 * np.pi * freq * rho.times), decimal=2
            )
    ...
    def test_cross_correlation_phi(self):
        ...
        phi = np.pi / 6.0
        signal = np.zeros((self.n_samples, 2))
        signal[:, 0] = 0.2 * np.sin(2.0 * np.pi * self.freq * self.times + phi)
        signal[:, 1] = 5.3 * np.cos(2.0 * np.pi * self.freq * self.times)
        # Convert signal to neo.AnalogSignal
        signal = neo.AnalogSignal(
            signal,
        ...
        assert_array_almost_equal(
            rho.magnitude[:, 0],
            np.sin(2.0 * np.pi * self.freq * rho.times + phi),
            decimal=2,
        )
    ...
    def test_cross_correlation_biased(self):
        signal = (
            np.c_[
                np.sin(2.0 * np.pi * self.freq * self.times),
                np.cos(2.0 * np.pi * self.freq * self.times),
            ]
            * pq.mV
        )
        signal = neo.AnalogSignal(
            signal, t_start=0.0 * pq.ms, sampling_rate=self.sampling_rate
        )
    ...
    def test_cross_correlation_coeff(self):
        ...
        assert_array_almost_equal(normalized.magnitude, target_numpy, decimal=3)
...
class ZscoreTestCase(unittest.TestCase):
    ...
    def test_zscore_single_dup(self):
        ...
        assert_array_almost_equal(result.magnitude, target.reshape(-1, 1), decimal=9)

        self.assertEqual(result.units, pq.Quantity(1.0 * pq.dimensionless))
    ...
    def test_zscore_array_annotations(self):
        ...
        self.assertDictEqual(signal.array_annotations, zscored.array_annotations)
    ...
    def test_zscore_single_multidim_inplace(self):
        ...
        m = np.mean(signal.magnitude, axis=0, keepdims=True)
        s = np.std(signal.magnitude, axis=0, keepdims=True)
        ground_truth = np.divide(
            signal.magnitude - m, s, out=np.zeros_like(signal.magnitude), where=s != 0
        )
        result = elephant.signal_processing.zscore(signal, inplace=True)
    ...
    def test_zscore_list_dup(self):
        ...
        assert_array_almost_equal(
            result[0].magnitude,
            np.transpose(np.vstack([target11, target12])),
            decimal=9,
        )
        assert_array_almost_equal(
            result[1].magnitude,
            np.transpose(np.vstack([target21, target22])),
            decimal=9,
        )
    ...
    def test_z_score_wrong_input(self):
        # wrong type
        self.assertRaises(
            TypeError, elephant.signal_processing.zscore, signal=[1, 2] * pq.uV
        )
        # units mismatch
        asig1 = neo.AnalogSignal([0, 1], units=pq.uV, sampling_rate=1 * pq.ms)
        asig2 = neo.AnalogSignal([0, 1], units=pq.V, sampling_rate=1 * pq.ms)
        self.assertRaises(
            ValueError, elephant.signal_processing.zscore, signal=[asig1, asig2]
        )
...
class ButterTestCase(unittest.TestCase):
    def test_butter_filter_type(self):
        ...
        noise = neo.AnalogSignal(
            np.random.normal(size=5000), sampling_rate=1000 * pq.Hz, units="mV"
        )
    ...
    def test_butter_filter_function(self):
        ...
        kwds = {
            "signal": noise,
            "highpass_frequency": 250.0 * pq.Hz,
        ...
        self.assertDictEqual(noise.array_annotations, filtered_noise.array_annotations)
    ...
    def test_butter_input_types(self):
        ...
        self.assertTrue(np.all(filtered_noise.magnitude[:, 0] == filtered_noise_np))
    ...
    def test_butter_multidim_input(self):
        ...
        noise_neo = neo.AnalogSignal(noise_pq.T, sampling_rate=1000.0 * pq.Hz)
        noise_neo1d = neo.AnalogSignal(noise_pq[0], sampling_rate=1000.0 * pq.Hz)
        filtered_noise_pq = elephant.signal_processing.butter(
            noise_pq, 250.0, sampling_frequency=1000.0
        )
        filtered_noise_neo = elephant.signal_processing.butter(noise_neo, 250.0)
        filtered_noise_neo1d = elephant.signal_processing.butter(noise_neo1d, 250.0)
        self.assertTrue(
            np.all(filtered_noise_pq.magnitude == filtered_noise_neo.T.magnitude)
        )
        self.assertTrue(
            np.all(
...
class HilbertTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.amplitude = np.array(
            [
                ...
                np.linspace(1, 10, len(time)),
                np.ones((len(time))),
                np.ones((len(time))) * 10.0,
            ]
        ).T
        self.phase = np.array(
            [
                (time * freq).simplified.magnitude * 2.0 * np.pi,
                (time * freq).simplified.magnitude * 2.0 * np.pi + np.pi / 2,
                (time * freq).simplified.magnitude * 2.0 * np.pi + np.pi,
                (time * freq).simplified.magnitude * 2.0 * 2.0 * np.pi,
            ]
        ).T
        ...
        sigs = np.vstack(
            [np.sin(phases), np.cos(phases), np.sin(2 * phases), np.cos(2 * phases)]
        )

        self.one_period = neo.AnalogSignal(
            sigs.T, units=pq.mV, sampling_rate=len(phases) * pq.Hz
        )
    ...
    def test_hilbert_pad_type_error(self):
        ...
        self.assertRaises(
            ValueError,
            elephant.signal_processing.hilbert,
            self.long_signals,
            padding=padding,
        )
    ...
    def test_hilbert_output_shape(self):
        ...
        output = elephant.signal_processing.hilbert(self.long_signals, padding=16384)
        self.assertEqual(np.shape(output), true_shape)
        self.assertEqual(output.units, pq.dimensionless)
    ...
    def test_hilbert_array_annotations(self):
        ...
        assert_array_equal(
            output.array_annotations["my_list"],
    ...
    def test_hilbert_theoretical_long_signals(self):
        ...
        for padding in ["nextpow", "none", 16384]:
            h = elephant.signal_processing.hilbert(self.long_signals, padding=padding)
            ...
            assert_array_almost_equal(
                real_value, self.long_signals.magnitude, decimal=14
            )
            ...
            phasediff[phasediff >= np.pi] = 2 * np.pi - phasediff[phasediff >= np.pi]

            # Calculate difference in amplitude between signal and original
            # amplitude
            amplitudediff = amplitude[ind1:ind2, :] - self.amplitude[ind1:ind2, :]
            #
    ...
    def test_hilbert_theoretical_one_period(self):
        ...
        for padding in ["nextpow", "none", 512]:
            h = elephant.signal_processing.hilbert(self.one_period, padding=padding)
            ...
            # The real part should be equal to the original long_signals:
            assert_array_almost_equal(
                real_value, self.one_period.magnitude, decimal=decimal
            )

            # The absolute value should be 1 everywhere, for this input:
            assert_array_almost_equal(
                amplitude, np.ones(self.one_period.magnitude.shape), decimal=decimal
            )
            ...
            # same interval:
            assert_array_almost_equal(
                phase[:256, 1], np.arange(0, np.pi, np.pi / 256), decimal=decimal
            )
            ...
            # time:
            assert_array_almost_equal(
                phase[:128, 3], np.arange(0, np.pi, np.pi / 128), decimal=decimal
            )
...
class WaveletTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.test_data = neo.AnalogSignal(
            self.test_data_arr.T * pq.mV,
            t_start=self.times[0] * pq.s,
            t_stop=self.times[-1] * pq.s,
            sampling_period=(1 / self.fs) * pq.s,
        )
    ...
    def test_wavelet_errors(self):
        ...
        kwds = {"signal": self.test_data, "frequency": [self.fs / 10, self.fs / 2]}
        self.assertRaises(
            ValueError, elephant.signal_processing.wavelet_transform, **kwds
        )
        kwds = {
        ...
        # nco is not positive
        kwds = {"signal": self.test_data, "frequency": self.fs / 10, "n_cycles": 0}
        self.assertRaises(
            ValueError, elephant.signal_processing.wavelet_transform, **kwds
        )
    ...
    def test_wavelet_io(self):
        ...
        wt = elephant.signal_processing.wavelet_transform(self.test_data, self.fs / 10)
        ...
        wt = elephant.signal_processing.wavelet_transform(self.test_data, self.wt_freqs)
        ...
        assert_array_almost_equal(wt_1freq[:, 0], wt_3freqs[:, 0, 0], decimal=12)
    ...
    def test_wavelet_phase(self):
        ...
        assert_array_almost_equal(
            np.exp(1j * phase), np.exp(1j * true_phase), decimal=6
        )
        ...
        assert_array_almost_equal(
            np.exp(1j * phase_padded), np.exp(1j * phase), decimal=9
        )
...
class DerivativeTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.test_data2 = np.vstack(
            [np.cos(2 * np.pi * self.times), np.sin(2 * np.pi * self.times)]
        ).T
        self.test_signal1 = neo.AnalogSignal(
            self.test_data1 * pq.mV,
            t_start=self.times[0] * pq.s,
            t_stop=self.times[-1] * pq.s,
            sampling_period=(1 / self.fs) * pq.s,
        )
        self.test_signal2 = neo.AnalogSignal(
            self.test_data2 * pq.mV,
            t_start=self.times[0] * pq.s,
            t_stop=self.times[-1] * pq.s,
            sampling_period=(1 / self.fs) * pq.s,
        )
    ...
    def test_derivative_invalid_signal(self):
        ...
        self.assertRaises(TypeError, elephant.signal_processing.derivative, **kwds)

    def test_derivative_units(self):
        """Test derivative returns AnalogSignal with correct units"""
        derivative = elephant.signal_processing.derivative(self.test_signal1)
        self.assertTrue(isinstance(derivative, neo.AnalogSignal))
        self.assertEqual(
            derivative.units, self.test_signal1.units / self.test_signal1.times.units
        )
    ...
    def test_derivative_times(self):
        ...
        self.assertEqual(
            derivative.sampling_period, 1 / self.fs * self.test_signal1.times.units
        )

        # test that all times are correct
        target_times = (
            self.times[:-1] * self.test_signal1.times.units
        ...
        assert_array_almost_equal(
            derivative.t_stop, target_times[-1] + derivative.sampling_period
        )

    def test_derivative_values(self):
        """Test derivative returns AnalogSignal with correct values"""
        derivative1 = elephant.signal_processing.derivative(self.test_signal1)
        derivative2 = elephant.signal_processing.derivative(self.test_signal2)
        ...
        # multi channel
        assert_array_almost_equal(
            derivative2.magnitude,
            np.vstack(
                [np.diff(self.test_data2[:, 0]), np.diff(self.test_data2[:, 1])]
            ).T
            / (1 / self.fs),
        )
...
class RAUCTestCase(unittest.TestCase):
    ...
    def test_rauc_invalid_signal(self):
        ...
        kwds = {"signal": np.arange(5)}
        self.assertRaises(ValueError, elephant.signal_processing.rauc, **kwds)
    ...
    def test_rauc_units(self):
        ...
        rauc = elephant.signal_processing.rauc(self.test_signal1)
        self.assertTrue(isinstance(rauc, pq.Quantity))
        self.assertEqual(
            rauc.units, self.test_signal1.units * self.test_signal1.times.units
        )
        ...
        self.assertEqual(
            rauc_arr.units, self.test_signal1.units * self.test_signal1.times.units
        )
    ...
    def test_rauc_times_without_overextending_bin(self):
        ...
        # test that all times are correct
        target_times = (
            np.arange(self.tmin, self.tmax, bin_duration.magnitude) * bin_duration.units
            + bin_duration / 2
        )
        ...
        assert_array_almost_equal(rauc_arr.t_stop, target_times[-1] + bin_duration)
    ...
    def test_rauc_values_one_bin(self):
        ...
        rauc1 = elephant.signal_processing.rauc(self.test_signal1)
        rauc2 = elephant.signal_processing.rauc(self.test_signal2)
        self.assertTrue(isinstance(rauc1, pq.Quantity))
        self.assertTrue(isinstance(rauc2, pq.Quantity))

        # single channel
        assert_array_almost_equal(rauc1.magnitude, np.array([6.36517679]))

        # multi channel
        assert_array_almost_equal(rauc2.magnitude, np.array([6.36517679, 6.36617364]))
    ...
    def test_rauc_mean_baseline(self):
        ...
        rauc1 = elephant.signal_processing.rauc(self.test_signal1, baseline="mean")
        rauc2 = elephant.signal_processing.rauc(self.test_signal2, baseline="mean")
        self.assertTrue(isinstance(rauc1, pq.Quantity))
        self.assertTrue(isinstance(rauc2, pq.Quantity))
    ...
    def test_rauc_median_baseline(self):
        ...
        rauc1 = elephant.signal_processing.rauc(self.test_signal1, baseline="median")
        rauc2 = elephant.signal_processing.rauc(self.test_signal2, baseline="median")
        self.assertTrue(isinstance(rauc1, pq.Quantity))
        self.assertTrue(isinstance(rauc2, pq.Quantity))
    ...
    def test_rauc_arbitrary_baseline(self):
        ...
        # single channel
        assert_array_almost_equal(rauc1.magnitude, np.array([6.41354725]))

        # multi channel
        assert_array_almost_equal(rauc2.magnitude, np.array([6.41354725, 6.41429810]))
    ...
    def test_rauc_time_slice(self):
        ...
        # single channel
        assert_array_almost_equal(rauc1.magnitude, np.array([0.16279006]))

        # multi channel
        assert_array_almost_equal(rauc2.magnitude, np.array([0.16279006, 0.26677944]))
