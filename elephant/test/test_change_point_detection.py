...
class FilterTestCase(unittest.TestCase):
    ...
    def test_filter_with_spiketrain_h05(self):
        ...
        self.assertRaises(
            ValueError, mft._filter, 0.8 * pq.s, 0.5 * pq.s, self.test_array
        )
...
class FilterProcessTestCase(unittest.TestCase):
    ...
    def test_filter_process_with_spiketrain_h05(self):
        ...
        res = mft._filter_process(
            0.5 * pq.s, 0.5 * pq.s, st, 2.01 * pq.s, np.array([[0.5], [1.7], [0.4]])
        )
...
class MultipleFilterAlgorithmTestCase(unittest.TestCase):
    ...
    def test_MultipleFilterAlgorithm_with_spiketrain_h05(self):
        ...
        res = mft.multiple_filter_test(
            [0.5] * pq.s, st, 2.1 * pq.s, 5, 100, time_step=0.1 * pq.s
        )
        assert_array_almost_equal(res, target, decimal=9)
    ...
    def test_MultipleFilterAlgorithm_with_quantities_h05(self):
        ...
        target = [self.targ_h05_dt05]
        res = mft.multiple_filter_test(
            [0.5] * pq.s, st, 2.1 * pq.s, 5, 100, time_step=0.5 * pq.s
        )
        self.assertNotIsInstance(res, pq.Quantity)
        assert_array_almost_equal(res, target, decimal=9)
    ...
    def test_MultipleFilterAlgorithm_with_longdata(self):
        ...
        def alternative_hypothesis(
            k1, teta1, c1, k2, teta2, c2, k3, teta3, c3, k4, teta4, T
        ):
        ...
        st = self.h1 = alternative_hypothesis(
            1, 1 / 4.0, 150, 2, 1 / 26.0, 30, 1, 1 / 36.0, 320, 2, 1 / 33.0, 200
        )[0]
        ...
        assert_allclose(result_concatenated[:3], target[:3], rtol=0, atol=5)
        print(
            "detected {0} cps: {1}".format(
                len(result_concatenated), result_concatenated
            )
        )
