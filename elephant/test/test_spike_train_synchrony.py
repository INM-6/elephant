...
from elephant.spike_train_synchrony import (
    Synchrotool,
    spike_contrast,
    _get_theta_and_n_per_bin,
...
class TestSpikeContrast(unittest.TestCase):
    def test_spike_contrast_random(self):
        ...
        poisson_process_1 = stgen.StationaryPoissonProcess(
            rate=10.0 * Hz, t_start=0.0 * ms, t_stop=10000.0 * ms
        )
        poisson_process_2 = stgen.StationaryPoissonProcess(
            rate=1.0 * Hz, t_start=0.0 * ms, t_stop=10000.0 * ms
        )
    ...
    def test_spike_contrast_double_duration(self):
        ...
        poisson_process = stgen.StationaryPoissonProcess(
            rate=10 * Hz, t_start=0.0 * ms, t_stop=10000.0 * ms
        )
    ...
    def test_spike_contrast_non_overlapping_spiketrains(self):
        ...
        spike_train_2 = stgen.StationaryPoissonProcess(
            rate=10 * Hz, t_start=5000.0 * ms, t_stop=10000.0 * ms
        ).generate_spiketrain()
        ...
        self.assertEqual(synchrony, 0.0)

    def test_spike_contrast_trace(self):
        np.random.seed(15)
        poisson_process = stgen.StationaryPoissonProcess(
            rate=10 * Hz, t_start=0.0 * ms, t_stop=1000.0 * ms
        )
        spike_train_1 = poisson_process.generate_spiketrain()
        spike_train_2 = poisson_process.generate_spiketrain()
        synchrony, trace = spike_contrast(
            [spike_train_1, spike_train_2], return_trace=True
        )
    ...
    def test_invalid_data(self):
        ...
        self.assertRaises(
            ValueError, spike_contrast, spiketrains, bin_shrink_factor=0.0
        )

        # invalid t_start, t_stop, and min_bin
        self.assertRaises(TypeError, spike_contrast, spiketrains, t_start=0)
        self.assertRaises(TypeError, spike_contrast, spiketrains, t_stop=1000)
        self.assertRaises(TypeError, spike_contrast, spiketrains, min_bin=0.01)

    def test_t_start_agnostic(self):
        np.random.seed(15)
        t_stop = 10 * second
        poisson_process = stgen.StationaryPoissonProcess(rate=10 * Hz, t_stop=t_stop)
        spike_train_1 = poisson_process.generate_spiketrain()
        spike_train_2 = poisson_process.generate_spiketrain()
        spiketrains = [spike_train_1, spike_train_2]
        ...
        t_shift = 20 * second
        spiketrains_shifted = [
            neo.SpikeTrain(st.times + t_shift, t_start=t_shift, t_stop=t_stop + t_shift)
    ...
    def test_get_theta_and_n_per_bin(self):
        spike_trains = [[1, 2, 3, 9], [1, 2, 3, 9], [1, 2, 2.5]]
        theta, n = _get_theta_and_n_per_bin(
            spike_trains, t_start=0, t_stop=10, bin_size=5
        )
    ...
    def test_spike_contrast_with_Izhikevich_network_auto(self):
        ...
        filepath_zip = download_datasets(repo_path=izhikevich_gin, checksum=checksum)
        unzip(filepath_zip)
        ...
        for network_simulations in networks_subset:
            for simulation in network_simulations.values():
                ...
                spiketrains = [
                    neo.SpikeTrain(
                        st, t_start=0 * second, t_stop=2 * second, units=second
                    )
...
class SynchrofactDetectionTestCase(unittest.TestCase):
    def _test_template(
        ...
        annotations = [st.array_annotations["complexity"] for st in spiketrains]
        ...
        synchrofact_obj.delete_synchrofacts(
            threshold=deletion_threshold, in_place=in_place, mode=mode
        )

        cleaned_spike_times = [st.times for st in spiketrains]

        for correct_st, cleaned_st in zip(correct_spike_times, cleaned_spike_times):
    ...
    def test_no_synchrofacts(self):
        ...
        spiketrains = [
            neo.SpikeTrain([1, 9, 12, 19] * pq.s, t_stop=20 * pq.s),
            neo.SpikeTrain([3, 7, 15, 17] * pq.s, t_stop=20 * pq.s),
        ]

        correct_annotations = np.array([[1, 1, 1, 1], [1, 1, 1, 1]])
    ...
    def test_spread_0(self):
        ...
        correct_annotations = np.array([[2, 1, 1, 1, 2, 1], [2, 1, 1, 1, 2, 1]])
    ...
    def test_spread_1(self):
        ...
        correct_annotations = np.array([[2, 2, 1, 3, 3, 1], [2, 2, 1, 3, 1, 1]])
    ...
    def test_n_equals_3(self):
        ...
        spiketrains = [
            neo.SpikeTrain([1, 1, 5, 10, 13, 16, 17, 19] * pq.s, t_stop=21 * pq.s),
            neo.SpikeTrain([1, 4, 7, 9, 12, 14, 16, 20] * pq.s, t_stop=21 * pq.s),
        ]

        correct_annotations = np.array(
            [[3, 3, 2, 2, 3, 3, 3, 2], [3, 2, 1, 2, 3, 3, 3, 2]]
        )
    ...
    def test_binning_for_input_with_rounding_errors(self):
        ...
        spiketrains = [
            neo.SpikeTrain(np.arange(1000) * pq.s / 30000, t_stop=0.1 * pq.s),
            neo.SpikeTrain(np.arange(2000, step=2) * pq.s / 30000, t_stop=0.1 * pq.s),
        ]
        ...
        correct_annotations = np.array([first_annotations, second_annotations])
    ...
    def test_correct_transfer_of_spiketrain_attributes(self):
        ...
        synchrofact_obj = Synchrotool(
            [spiketrain], spread=0, sampling_rate=sampling_rate, binary=False
        )
        synchrofact_obj.delete_synchrofacts(mode="delete", in_place=True, threshold=2)
